...
| 阶段 3：安全图片上传与文件管理 | completed | `11ca19e` |
| 阶段 4：真实 YOLO 目标检测 | completed | 本次提交 |
...
- 定义目标检测适配器、YOLO11n 实现和统一推理服务。
- 使用本地 YOLO11n 通用预训练权重完成真实 CPU 推理。
- 目标检测任务真实流转为 `pending → running → completed/failed`。
- 新增 `task_results` 表和迁移版本 `20260708_0003`，保存模型、设备和结构化检测结果。
- 保存带框 PNG 结果图，并通过受控文件 API 返回。
- 前端仅开放目标检测，其他三类任务继续标记为 `planned`。
- 任务详情展示原图、结果图、类别、置信度、像素坐标框、模型版本、设备和耗时。
- 通用权重只用于工程闭环验证，不描述为遥感专项模型。
...
- 后端：`17 passed`，包含无目标、模型缺失、推理异常和真实 YOLO11n CPU 集成测试。
- 前端：`npm.cmd run build` 通过。
- 迁移：本地 SQLite 为 `20260708_0003 (head)`，`db check` 未检测到差异。
- 真实样例：CPU 推理完成并检测到 1 个 `person`，保存结构化结果和带框图片。
- 浏览器：结果页显示 `completed`、`real_model`、结果图和检测列表，控制台无错误。
- 当前状态：用户本地验收通过，阶段 4 可以提交。
